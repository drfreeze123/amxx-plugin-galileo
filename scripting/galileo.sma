...
new g_voteWeightFlags[32], g_rtvDisableFlags[32];
...
new cvar_rtvWait, cvar_rtvRatio, cvar_rtvCommands, cvar_rtvReminder, cvar_rtvDisableFlags;
...
nomination_attempt(id, nomination[]) // (playerName[], &phraseIdx, matchingSegment[])
{
	...
	new mapIdx, nominationMap[32], matchCnt = 0, matchIdx = -1, info[7], choice[64], disabledReason[16];
	for (mapIdx = 0; mapIdx < g_nominationMapCnt && matchCnt <= MAX_NOM_MATCH_CNT; ++mapIdx)
	{
		...
		if (contain(nominationMap, nomination) > -1)
		{
			...
			formatex(info, sizeof(info)-1, "%i", mapIdx);
			...
			// disable if the map is the current map
...
public nomination_handleMatchChoice(id, menu, item)
{
	...
	menu_item_getinfo(g_nominationMatchesMenu[id], item, access, info, sizeof(info)-1, _, _, callback);
 
 	mapIdx = str_to_num(info);
...
public vote_handleDisplay()
{
	...
	if (g_voteStatus & VOTE_IS_RUNOFF)
	{
		copy(voteType, sizeof(voteType)-1, "Runoff Vote");
	}
	else if (g_voteStatus & VOTE_IS_EARLY)
	{
		copy(voteType, sizeof(voteType)-1, "Early Vote");
	}
	else if (g_voteStatus & VOTE_FORCED)
	{
		copy(voteType, sizeof(voteType)-1, "Forced Vote");
	}
	else
	{
		copy(voteType, sizeof(voteType)-1, "End of Map Vote");
	}
		
	new players[32], playerCnt;
	get_players(players, playerCnt, "ch"); // skip bots and hltv

	new id, hasMenu, menuid, newmenuid, menupage, menuid2, menukeys2, name[32];
...
public vote_display(arg[3])
{
	...
	if (id > 0)
	{
		...
		if (showStatus == SHOWSTATUS_VOTE)
		{
			...
			vote_showMenu(id, allKeys, menuDirty, max(1, g_voteDuration));
		}
	}
	else
	{
		...
		for (new playerIdx = 0; playerIdx < playerCnt; ++playerIdx)
		{
			...
			if (g_voted[id] == false && !isVoteOver)
			{
				...
				vote_showMenu(id, keys, menuClean, g_voteDuration);
			}
			else 
			{
				if ((isVoteOver && showStatus) || (showStatus == SHOWSTATUS_VOTE && g_voted[id]))
				{
					...
					vote_showMenu(id, allKeys, menuDirty, (isVoteOver) ? 5 : max(1, g_voteDuration));
				}
			}
			// dbg code ----
			if (id == 1)
...
public vote_rock(id)
{
	// if RTV is to be disabled when players with a specific flag are in-game, disallow if needed
	if (g_rtvDisableFlags[0])
	{
		new player[32], playerCnt;
		get_players(player, playerCnt, "ch");	// skip bots and hltv
		for (new idxPlayer = 0; idxPlayer < playerCnt; ++idxPlayer)
		{
			if (has_flag(player[idxPlayer], g_rtvDisableFlags))
			{
				client_print(id, print_chat, "%L", id, "GAL_ROCK_FAIL_ADMINPRESENT");
				return;
			}
		}
	}
...
map_listAll(id)
{
	if (g_nominationMapCnt == 0)
	{
		con_print(id, "%L", id, "GAL_LISTMAPS_NOMAPS");
		return;
	}
